...
def sort_author_keyboard(authors):
    ...
    for i in authors:
        builder_authors.add(InlineKeyboardButton(
            text=i, callback_data=f"sort_voices_authors {i}"))
...
def sort_tags_keyboard(sorted_tags):
    ...
    for i in sorted_tags:
        builder_tags.add(InlineKeyboardButton(
            text=i, callback_data=f"sort_voices_tags {i}"))
    builder_tags.add(InlineKeyboardButton(
        text="🔼Меню🔼", callback_data=f"menu"))
...
async def list_of_authors(message: Message):
    if message.from_user.id in ADMINS:
        async with httpx.AsyncClient() as client:
            response = await client.get(os.getenv("HOST") + "/bot/authors/all")
...
async def list_of_tags(callback: CallbackQuery):
    async with httpx.AsyncClient() as client:
        response = await client.get(os.getenv("HOST") + "/bot/tags/sorted?author=" + callback.data.replace("sort_voices_authors ", ""))
        read_tags_by_authors = response.json()
    sorted_tags_by_authors = list(
        set([j for i in read_tags_by_authors for j in i["tags"]]))
...
async def sort_tags(callback: CallbackQuery):
    async with httpx.AsyncClient() as client:
        response = await client.get(os.getenv("HOST") + "/bot/tags/?tag=" + callback.data.replace("sort_voices_tags ", ""))
    ...
    await bot.send_voice(
        ...
        "Описание голосового: " +
        sort_tags.voices_by_tags[0]["description"] + "\n",
