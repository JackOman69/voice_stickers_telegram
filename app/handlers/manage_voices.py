...
def get_keyboard(id):
    ...
    manage_keyboard.row(
        InlineKeyboardButton(
            text="Посмотреть", callback_data=f"description_show {id}"),
        InlineKeyboardButton(
            text="Скрыть", callback_data=f"description_hide {id}")
    )
    manage_keyboard.row(
        InlineKeyboardButton(
            text="Изменить", callback_data=f"description_edit {id}"),
        InlineKeyboardButton(
            text="Удалить", callback_data=f"description_delete {id}")
    )
    manage_keyboard.row(
        InlineKeyboardButton(text="⬅️", callback_data=f"tags_back"),
        InlineKeyboardButton(
            text=f"{amount + 1}/{len(sort_by_tags.sort_tags.voices_by_tags)}", callback_data=f"tags_count"),
...
async def show_description(callback: CallbackQuery):
    async with httpx.AsyncClient() as client:
        response = await client.get(os.getenv("HOST") + "/bot/id/?id=" + callback.data.replace("description_show ", ""))
    ...
    await bot.edit_message_caption(
        ...
        show_data_voice["author"] + "\n" + "ID Автора: " +
        str(show_data_voice["admin_author_id"]),
...
async def hide_description(callback: CallbackQuery):
    async with httpx.AsyncClient() as client:
        response = await client.get(os.getenv("HOST") + "/bot/id/?id=" + callback.data.replace("description_hide ", ""))
...
async def delete_description(callback: CallbackQuery):
    ...
    async with httpx.AsyncClient() as client:
        await client.delete(os.getenv("HOST") + "/bot/delete/?id=" + callback.data.replace("description_delete ", ""))
    ...
    try:
        if amount < 0:
            ...
            await bot.send_voice(
                ...
                "Описание голосового: " +
                sort_by_tags.sort_tags.voices_by_tags[amount]["description"] + "\n",
                reply_markup=get_keyboard(
                    sort_by_tags.sort_tags.voices_by_tags[amount]["id"])
            )
            await bot.delete_message(callback.from_user.id, callback.message.message_id)
    except IndexError:
